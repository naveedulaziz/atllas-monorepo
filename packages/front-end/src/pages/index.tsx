...
export async function getServerSideProps(ctx: GetServerSidePropsContext) {
  ...
  const cookieObj = new URLSearchParams(
    ctx.req.headers.cookie?.replaceAll("&", "%26").replaceAll("; ", "&")
  );
  const token = cookieObj.get("SESSION_TOKEN");

  if (token) {
    const authRes = await fetch(
      `http://${process.env.BACK_END_HOST}:5000/auth`,
      {
        method: "GET",
        headers: {
  ...
  return (
    <>
      ...
      <main className={clsx("w-full h-full", inter.className)}>
        ...
        <div className="p-4">
          <p className="text-neutral-500">{`How ya goin, ${sess?.displayName || sess?.username || "stranger"
            }?`}</p>
        </div>
      </main>
    </>
